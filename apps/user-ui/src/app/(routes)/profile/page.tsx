...
const ProfilePageContent = () => {
  ...
  const stats = {
    ...
    processing: orders.filter(
      (o: any) => o.status === "Processing" || o.deliveryStatus === "Ordered",
    ).length,
    completed: orders.filter((o: any) => o.deliveryStatus === "Delivered")
      .length,
  };
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="hidden xl:flex flex-col gap-4 w-[320px]">
            {[
              ...
              <div
                key={i}
                className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex items-start gap-4 hover:shadow-md transition-all cursor-pointer group"
              >
                <div
                  className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center ${widget.bg} ${widget.color} group-hover:scale-110 transition-transform`}
                >
                  <widget.icon size={20} />
                </div>
                <div>
