...
import { X, CreditCard, Wallet, Banknote, ShieldCheck, CheckCircle2, ArrowLeft, Loader2 } from "lucide-react";
...
const stripePromise = loadStripe(process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || "pk_test_51PXzbSSGnmKVDMGgzYASGjq7ktUKr0moWbKGzm9cB1EzYl196t4LRvgU9jrI2IE5JhpLKQos2lEHPAIdQGJMGVq00VNk2qv95");
...
const PaymentModal = ({ isOpen, onClose, onBack, onComplete, sessionId, sessionData, totalAmount }: PaymentModalProps) => {
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-2xl h-[90vh] md:h-auto rounded-[40px] shadow-2xl overflow-hidden animate-slide-up flex flex-col">
        ...
        <div className="px-10 py-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/50 shrink-0">
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Checkout Payment</h2>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Order Total: ${totalAmount.toFixed(2)}</p>
          </div>
             ...
             {paymentMethods.map((method) => (
               ...
               >
                 <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                   selectedMethod === method.id ? "bg-[#47718F] text-white" : `${method.bgColor} ${method.color}`
                 }`}>
                    <method.icon size={20} />
                 </div>
                 <div className="text-center">
                   <h4 className="text-sm font-black text-slate-800 tracking-tight">{method.title}</h4>
            ...
            {selectedMethod === "stripe" ? (
              isLoadingSecret ? (
                <div className="py-20 flex flex-col items-center justify-center gap-4">
                  ...
                  <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Connecting to Stripe Secure...</p>
                </div>
                ...
                <div className="text-center py-10 bg-red-50 rounded-[32px] border border-red-100">
                   <p className="text-sm text-red-500 font-bold">Failed to load payment system. Please try again.</p>
                   <button onClick={fetchClientSecret} className="mt-4 text-xs font-black text-[#47718F] uppercase underline tracking-widest">Retry Connection</button>
                </div>
              )
            ) : (
              <div className="space-y-8 animate-fade-in">
                <div className="bg-emerald-50/50 p-10 rounded-[40px] border-2 border-emerald-100 text-center space-y-4">
                   ...
                   <div>
                      <h3 className="text-xl font-black text-slate-800">Cash on Delivery</h3>
                      <p className="text-sm text-slate-500 mt-2 max-w-xs mx-auto font-medium">
                        You can pay with cash or card when your order arrives at your doorstep.
                      </p>
                   </div>
                </div>
        ...
        <div className="px-10 py-8 border-t border-slate-50 bg-slate-50/30 flex items-center justify-between shrink-0">
           ...
           <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em]">PCI DSS Compliant</span>
           </div>
        </div>
      </div>
    </div>
